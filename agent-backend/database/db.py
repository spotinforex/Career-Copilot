...
class CareerCopilotDB:
    def __init__(self, database_url: str | None = None):
        self.database_url = database_url or get_secret_value("DATABASE_URL")
        self.conn = None
    ...
    def get_or_create_user(self, email: str):
        """Create or fetch a user row from the database using the authenticated email."""
        if not email:
            raise ValueError("Email is required to create a database user")

        existing = self.fetch_one("users", where={"email": email}, columns="id, email")
        if existing:
            return existing

        return self.insert("users", {"email": email}, returning="id, email")
    ...
    def append_resume_edit(self, user_id, role_tag, edit, embed_fn=None):
        ...
        content.setdefault("edits", []).append(edit)

        self.update("resumes", {"content": json.dumps(content), "updated_at": "now()"}, {"id": resume["id"]})
